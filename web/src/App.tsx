...
function Downloads({ jobs, halt, refresh, onError, onReveal }: { jobs: DownloadJob[]; halt: QueueHalt | null; refresh: () => Promise<void>; onError: (e: unknown) => void; onReveal: (target: string) => void }) {
  const queueTop = useRef<HTMLElement>(null);
  const changePage = (value: number) => { setPage(value); queueTop.current?.scrollIntoView({ block: "start" }); };
  const [query, setQuery] = useState("");
  ...
  useEffect(() => setPage(1), [query, status, sort, direction, dateField, from, to, pageSize]);
  ...
  return <section ref={queueTop} className="downloads-page"><div className="download-title"><Heading eyebrow={t("downloads.eyebrow")} title={t("downloads.title")}/><button disabled={!jobs.some((job) => job.status === "completed")} onClick={() => action(api.clearCompleted)}><Trash2/> {t("downloads.clearCompleted")}</button></div>{halt && <div className="queue-halt" role="status">{serverText(halt.messageKey, halt.message)} {t("downloads.haltResumes")}</div>}<div className="summary"><div><b>{jobs.length}</b><span>{t("downloads.itemsLabel")}</span></div><div><b>{active.length}</b><span>{t("downloads.running")}</span></div><div><b>{speed(totalSpeed)}</b><span>{t("downloads.totalSpeed")}</span></div></div><div className="queue-tools">
    ...
    <label>{t("downloads.from")}<input type="date" value={from} max={to || undefined} onChange={(e) => setFrom(e.target.value)}/></label>
    <label>{t("downloads.to")}<input type="date" value={to} min={from || undefined} onChange={(e) => setTo(e.target.value)}/></label>
    <button onClick={() => { setQuery(""); setStatus(""); setFrom(""); setTo(""); }}>{t("downloads.reset")}</button>
  </div><div className="panel downloads" tabIndex={0} role="region" aria-label={t("downloads.queueLabel")}><div className="download-head"><span>{t("downloads.colName")}</span><span>{t("downloads.colStatus")}</span><span>{t("downloads.colProgress")}</span><span>{t("downloads.colSpeed")}</span><span>{t("downloads.colActions")}</span></div>{visible.map((job)=><div className="download-row" key={job.id}><div className="download-job">{job.status === "completed" && job.target ? <button className="link-button job-link" title={t("downloads.showInLibrary")} onClick={() => onReveal(job.target)}>{job.title}</button> : <strong>{job.title}</strong>}<small>{job.target || (job.pending ? t("downloads.sourcePickedLater") : "")}{job.error ? ` · ${serverText(job.errorKey, job.error, job.errorVars)}`:""}</small><dl className="queue-times">{(["createdAt", "startedAt", "completedAt"] as const).map((field) => <div key={field}><dt>{t(`downloads.${field}`)}</dt><dd>{formatDate(job[field])}</dd></div>)}<div><dt>{t("downloads.duration")}</dt><dd>{duration(job) == null ? "—" : t("downloads.durationValue", { hours: Math.floor(duration(job)! / 3600000), minutes: Math.floor(duration(job)! / 60000) % 60, seconds: Math.floor(duration(job)! / 1000) % 60 })}</dd></div></dl></div><span className={`job-status ${job.status}`}>{statusLabel(job.status)}</span><div className="download-progress"><span>{job.status === "waiting" ? `${job.debridProgress ?? 0} %` : `${bytes(job.received)} / ${bytes(job.total)}`}</span><div className="progress"><i style={{width:`${job.status === "waiting" ? Math.min(100, job.debridProgress ?? 0) : job.total ? Math.min(100, job.received/job.total*100):0}%`}}/></div></div><span className="download-speed">{speed(job.speed)}{job.segments && job.segments > 1 ? <i className="segment-tag" title={t("downloads.segments", { count: job.segments })}>{`\u00d7${job.segments}`}</i> : null}<small>{eta(job)}</small></span><div className="queue-actions">{job.status === "completed" && job.target && <button title={t("downloads.showInLibrary")} onClick={() => onReveal(job.target)}><HardDrive/></button>}<button title={t("downloads.moveUp")} disabled={sort !== "order" || direction !== "asc" || job.order === 0 || job.status === "downloading"} onClick={() => action(() => api.moveDownload(job.id, -1))}><ArrowUp/></button><button title={t("downloads.moveDown")} disabled={sort !== "order" || direction !== "asc" || job.order === jobs.length - 1 || job.status === "downloading"} onClick={() => action(() => api.moveDownload(job.id, 1))}><ArrowDown/></button>{job.status === "downloading" || job.status === "queued" || job.status === "waiting" ? <button title={t("player.pause")} onClick={() => action(() => api.downloadAction(job.id,"pause"))}><Pause/></button> : job.status === "paused" ? <button title={t("library.continue")} onClick={() => action(() => api.downloadAction(job.id,"resume"))}><Play/></button> : job.status === "failed" ? <button title={t("downloads.retry")} onClick={() => action(() => api.downloadAction(job.id,"retry"))}><RefreshCw/></button> : null}<button className="danger" title={t("downloads.removeFromQueue")} onClick={() => action(() => api.removeDownload(job.id))}><Trash2/></button></div></div>)}{!jobs.length && <Empty icon={<Download/>} title={t("downloads.emptyTitle")} text={t("downloads.emptyText")}/>}{!!jobs.length && !filtered.length && <p>{t("downloads.noMatches")}</p>}</div><div className="queue-pagination"><label>{t("downloads.pageSize")}<select value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))}>{[20, 50, 100].map((size) => <option key={size}>{size}</option>)}</select></label><span role="status">{t("downloads.page", { page: currentPage, pages, count: filtered.length })}</span><button disabled={currentPage <= 1} onClick={() => changePage(currentPage - 1)}>{t("downloads.previous")}</button><button disabled={currentPage >= pages} onClick={() => changePage(currentPage + 1)}>{t("downloads.next")}</button></div></section>;
}
