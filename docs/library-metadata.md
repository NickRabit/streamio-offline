...
- `store.libraryMeta` is `Record<string, { type: string; id: string }>` in `data/state.json`. Keys are relative paths. `knownTitle()` walks parents. Delete/rename remaps `favorites`, `libraryMeta`, and `progress` only.
