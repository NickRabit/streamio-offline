...
export class DownloadQueue {
  ...
  private async download(job: DownloadJob) {
    const controller = new AbortController(); this.active.set(job.id, controller); job.status = "downloading"; job.error = undefined; job.updatedAt = new Date().toISOString(); await this.save();
    ...
    try {
      ...
      if (job.total && job.received !== job.total) throw new Error(`Stažená velikost nesouhlasí (${job.received} / ${job.total}).`); await rename(partial, target); job.status = "completed"; job.speed = 0;
    } catch (error) { job.speed = 0; const message = error instanceof Error ? error.message : String(error); if (this.pauseRequested.has(job.id)) job.status = "paused"; else if (/terminated|aborted|ECONNRESET|socket|fetch failed/i.test(message) && (job.retryCount ?? 0) < 3) { job.retryCount = (job.retryCount ?? 0) + 1; job.status = "queued"; job.error = `Přerušené spojení, opakuji (${job.retryCount}/3)…`; retryScheduled = true; setTimeout(() => this.pump(), 1500 * job.retryCount); } else { job.status = "failed"; job.error = message; } }
    finally { this.pauseRequested.delete(job.id); job.updatedAt = new Date().toISOString(); this.active.delete(job.id); await this.save(); if (!retryScheduled) this.pump(); }
  }
}
