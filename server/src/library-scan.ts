...
import {
  autoAccept, cacheFieldsFromMeta, knownTitleOf, pickSuggestion, scoreHit, viewMeta,
...
export class LibraryScan {
  ...
  private async processUnit(key: string) {
    ...
    try {
      ...
      if (knownTitleOf(key, records)?.id) { await this.finishUnit("skipped"); return; }
      if (viewMeta(records[key])?.locked) { await this.finishUnit("skipped"); return; }
      ...
      if (viewMeta(after[key])?.locked || knownTitleOf(key, after)?.id) { await this.finishUnit("skipped"); return; }
      ...
      if (identified.accept) {
        ...
        await this.opts.updateMeta((metaMap, suggestions) => {
          if (viewMeta(metaMap[key])?.locked || knownTitleOf(key, metaMap)?.id) return;
          metaMap[key] = {
  ...
  private async identify(unit: TitleUnit, parsed: ReturnType<typeof parseMediaPath>) {
    ...
    const hits = found.items.map((item) => scoreHit(parsed, item, unit.kind));
